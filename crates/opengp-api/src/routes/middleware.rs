...
pub(super) fn patient_request_to_new_data(
    ...
    Ok(NewPatientData {
        ihi: payload.ihi.map(Ihi::new_lenient),
        medicare_number: payload.medicare_number.map(MedicareNumber::new_lenient),
        medicare_irn: payload.medicare_irn,
        medicare_expiry: payload.medicare_expiry,
        title: payload.title,
        ...
        address: Address {
            ...
            country: payload.country.unwrap_or_else(|| "Australia".to_string()),
        },
        phone_home: payload.phone_home.map(PhoneNumber::new_lenient),
        phone_mobile: payload.phone_mobile.map(PhoneNumber::new_lenient),
        email: payload.email,
        emergency_contact: match payload.emergency_contact_name {
            Some(name) => Some(EmergencyContact {
                name,
                phone: payload.emergency_contact_phone.unwrap_or_default(),
                relationship: payload.emergency_contact_relationship.unwrap_or_default(),
            }),
            None => None,
        },
...
pub(super) fn patient_request_to_update_data(
    ...
    Ok(UpdatePatientData {
        ...
        address: if payload.address_line1.is_some() || payload.suburb.is_some() {
            Some(Address {
                line1: payload.address_line1,
                line2: payload.address_line2,
                suburb: payload.suburb,
