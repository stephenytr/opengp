...
impl App {
    pub async fn poll_api_tasks(&mut self) {
            ...
            match handle.await {
                ...
                Ok(Err(err)) => match err {
                    crate::api::ApiClientError::ServerUnavailable(message) => {
                        let retry_operation = retry_login
                            .map(|(username, password)| RetryOperation::Login { username, password });
        ...
        {
            #[allow(clippy::expect_used)]
            let handle = self.appointment_list_fetch_task.take().expect("task exists");
        ...
        {
            #[allow(clippy::expect_used)]
            let handle = self.practitioners_list_fetch_task.take().expect("task exists");
...
mod tests {
    ...
    fn pending_form_data_is_preserved_while_logged_out_and_available_after_relogin() {
        ...
        app.pending_patient_data = Some(crate::ui::app::PendingPatientData::New(
            opengp_domain::domain::patient::NewPatientData {
                ...
                phone_home: None,
                phone_mobile: Some(opengp_domain::domain::patient::PhoneNumber::new_lenient(
                    "0400000000".to_string(),
                )),
                email: None,
