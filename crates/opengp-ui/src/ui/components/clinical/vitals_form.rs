...
fn build_validation_rules(
    healthcare_config: &HealthcareConfig,
) -> HashMap<String, ValidationRules> {
    let mut rules = HashMap::new();

    // Helper to get range from config or use fallback
    let get_range = |key: &str, fallback_min: f64, fallback_max: f64| {
        healthcare_config
            .get_vital_sign(key)
            .map(|r| (r.min, r.max))
    ...
    let (systolic_min, systolic_max) = get_range("systolic_bp", 50.0, 300.0);
    rules.insert(
    ...
    let (diastolic_min, diastolic_max) = get_range("diastolic_bp", 20.0, 200.0);
    rules.insert(
    ...
    let (heart_rate_min, heart_rate_max) = get_range("heart_rate", 20.0, 300.0);
    rules.insert(
    ...
    let (respiratory_rate_min, respiratory_rate_max) = get_range("respiratory_rate", 4.0, 60.0);
    rules.insert(
    ...
    let (temperature_min, temperature_max) = get_range("temperature", 30.0, 45.0);
    rules.insert(
    ...
    let (o2_sat_min, o2_sat_max) = get_range("o2_saturation", 50.0, 100.0);
    rules.insert(
    ...
    let (height_min, height_max) = get_range("height", 30.0, 300.0);
    rules.insert(
    ...
    let (weight_min, weight_max) = get_range("weight", 0.5, 700.0);
    rules.insert(
...
mod tests {
    ...
    fn create_test_form() -> VitalSignsForm {
        let theme = Theme::dark();
        let healthcare_config = HealthcareConfig::default();
        VitalSignsForm::new(theme, healthcare_config)
    }
    ...
    #[test]
    fn test_vitals_form_uses_config_ranges_for_validation() {
        let mut config = HealthcareConfig::default();

        config.vital_signs.insert(
            "systolic_bp".to_string(),
            opengp_config::healthcare::VitalSignRange {
                min: 80.0,
                max: 180.0,
                unit: "mmHg".to_string(),
            },
        ...
        let theme = Theme::dark();
        let mut form = VitalSignsForm::new(theme, config);

        form.set_value(VitalSignsFormField::SystolicBp, "150".to_string());
        form.set_value(VitalSignsFormField::HeartRate, "85".to_string());

        let valid = FormNavigation::validate(&mut form);
        assert!(valid, "Values within config ranges should be valid");
        assert!(!form.has_errors());
    }

    #[test]
    fn test_vitals_form_rejects_values_outside_config_ranges() {
        let mut config = HealthcareConfig::default();
        ...
        form.set_value(VitalSignsFormField::SystolicBp, "250".to_string());

        let valid = FormNavigation::validate(&mut form);
        assert!(!valid, "Values outside config ranges should be invalid");
        assert!(form.error(VitalSignsFormField::SystolicBp).is_some());
    }

    #[test]
    fn test_vitals_form_uses_fallback_ranges_when_config_missing() {
        ...
        form.set_value(VitalSignsFormField::SystolicBp, "120".to_string());
        form.set_value(VitalSignsFormField::DiastolicBp, "80".to_string());

        let valid = FormNavigation::validate(&mut form);
        assert!(valid, "Should use fallback ranges when config is empty");
        assert!(!form.has_errors());
    }
}
