...
use crate::ui::widgets::{list_handle_key, list_handle_mouse, LoadingState, ScrollableState};
...
impl ConsultationList {
    ...
    pub fn with_consultations(consultations: Vec<Consultation>, theme: Theme) -> Self {
        ...
        Self {
            consultations,
            scrollable: ScrollableState::with_items(count),
            loading: false,
            loading_state: LoadingState::new().message("Loading consultations..."),
            theme,
            practitioner_names: std::collections::HashMap::new(),
    ...
    pub fn selected(&self) -> Option<&Consultation> {
        self.consultations.get(self.scrollable.selected_index())
    }
    ...
    pub fn select(&mut self, index: usize) {
        if index < self.consultations.len() {
            let offset = index as isize - self.scrollable.selected_index() as isize;
            self.scrollable.move_by(offset);
        }
    }

    pub fn selected_index(&self) -> usize {
        self.scrollable.selected_index()
    }
    ...
    pub fn handle_key(
        ...
        self.handle_key_with_visible_rows(key, 15)
    }

    pub fn handle_key_with_visible_rows(
        &mut self,
        key: crossterm::event::KeyEvent,
        visible_rows: usize,
    ) -> Option<ConsultationListAction> {
        ...
        if let Some(_nav_action) = list_handle_key(key, &mut self.scrollable, visible_rows) {
            return Some(ConsultationListAction::Select(
                self.scrollable.selected_index(),
            ));
        }
        ...
        match key.code {
            ...
            KeyCode::Char('f') => {
                ...
                Some(ConsultationListAction::Select(
                    self.scrollable.selected_index(),
                ))
            }
            KeyCode::Esc => {
    ...
    pub fn handle_mouse(
        ...
        if let Some(_nav_action) = list_handle_mouse(
            mouse,
            area,
            HEADER_HEIGHT,
            &mut self.scrollable,
...
impl Widget for ConsultationList {
    fn render(mut self, area: Rect, buf: &mut Buffer) {
        ...
        self.scrollable.set_item_count(self.consultations.len());
        let scroll_offset = self.scrollable.scroll_offset();
        ...
        let rows: Vec<Row> = self
            ...
            .map(|(i, consultation)| {
                ...
                let style = if actual_index == self.scrollable.selected_index() {
                    Style::default()
                        .bg(self.theme.colors.selected)
...
mod tests {
    ...
    fn test_consultation_list_navigation() {
        ...
        list.scrollable.set_item_count(list.consultations.len());
