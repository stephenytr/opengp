//! Clinical Component Module
//!
//! Clinical records UI components for patient consultations, allergies,
//! medical history, vital signs, social history, and family history.

pub mod allergy_detail_modal;
pub mod allergy_form;
pub mod allergy_list;
pub mod allergy_state;
pub mod consultation_detail;
pub mod consultation_detail_modal;
pub mod consultation_form;
pub mod consultation_list;
pub mod consultation_state;
pub mod family_history_detail_modal;
pub mod family_history_form;
pub mod family_history_list;
pub mod family_history_state;
pub mod medical_history_detail_modal;
pub mod medical_history_form;
pub mod medical_history_list;
pub mod medical_history_state;
pub mod patient_summary;
pub mod social_history;
pub mod social_history_state;
pub mod state;
pub mod sub_state;
pub mod vitals_detail_modal;
pub mod vitals_form;
pub mod vitals_list;
pub mod vitals_state;

pub use allergy_detail_modal::{AllergyDetailModal, AllergyDetailModalAction};
pub use allergy_form::{AllergyForm, AllergyFormAction};
pub use allergy_list::{AllergyList, AllergyListAction};
pub use allergy_state::AllergyState;
pub use consultation_detail::{ConsultationDetail, ConsultationDetailAction};
pub use consultation_detail_modal::{ConsultationDetailModal, ConsultationDetailModalAction};
pub use consultation_form::{ConsultationForm, ConsultationFormAction};
pub use consultation_list::{ConsultationList, ConsultationListAction};
pub use consultation_state::ConsultationState;
pub use family_history_detail_modal::{FamilyHistoryDetailModal, FamilyHistoryDetailModalAction};
pub use family_history_form::{FamilyHistoryForm, FamilyHistoryFormAction};
pub use family_history_list::{FamilyHistoryList, FamilyHistoryListAction};
pub use family_history_state::FamilyHistoryState;
pub use medical_history_detail_modal::{
    MedicalHistoryDetailModal, MedicalHistoryDetailModalAction,
};
pub use medical_history_form::{MedicalHistoryForm, MedicalHistoryFormAction};
pub use medical_history_list::{MedicalHistoryList, MedicalHistoryListAction};
pub use medical_history_state::MedicalHistoryState;
pub use patient_summary::PatientSummaryComponent;
pub use social_history::{SocialHistoryAction, SocialHistoryComponent};
pub use social_history_state::SocialHistoryState;
pub use state::{ClinicalFormView, ClinicalState, ClinicalView, ClinicalSubDomain};
pub use sub_state::ClinicalSubState;
pub use vitals_detail_modal::{VitalsDetailModal, VitalsDetailModalAction};
pub use vitals_form::{VitalSignsForm, VitalSignsFormAction};
pub use vitals_list::{VitalSignsList, VitalSignsListAction};
pub use vitals_state::VitalsState;
