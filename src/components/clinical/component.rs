...
use crate::domain::clinical::{ClinicalService, NewAllergyData};
...
use super::state::{
    AllergyFormField, AllergyFormState, ClinicalView, FamilyHistoryFormState, MedicalHistoryFormState,
    ...
    async fn update(&mut self, action: Action) -> Result<Option<Action>> {
        match action {
            ...
            Action::ClinicalAllergyAdd(patient_id) => {
                self.allergy_form = AllergyFormState {
                    patient_id: Some(patient_id),
                    allergen: String::new(),
                    allergy_type: String::new(),
                    severity: String::new(),
                    reaction: String::new(),
            ...
            Action::ClinicalAllergySave => {
                if let Some(patient_id) = self.allergy_form.patient_id {
                    let data = NewAllergyData {
                        patient_id,
                        allergen: self.allergy_form.allergen.clone(),
                        allergy_type: self.allergy_form.allergy_type(),
                        severity: self.allergy_form.severity(),
                        reaction: if self.allergy_form.reaction.is_empty() {
                            None
                        } else {
                            Some(self.allergy_form.reaction.clone())
                        },
                        onset_date: self.allergy_form.parse_onset_date(),
                        notes: if self.allergy_form.notes.is_empty() {
                            None
                        } else {
                            Some(self.allergy_form.notes.clone())
                        },
                    };

                    // Use seeded user ID as placeholder - proper auth will provide this
                    let system_user_id = Uuid::parse_str("a1b2c3d4e5f64789a1b2c3d4e5f64789").unwrap();
                    match self.clinical_service.add_allergy(data, system_user_id).await {
                        Ok(_) => {
                            self.load_patient_data(patient_id).await;
                            self.current_view = ClinicalView::AllergyList;
                        }
                        Err(e) => {
                            self.error_message = Some(format!("Failed to save allergy: {}", e));
                        }
                    }
                }
                Ok(Some(Action::Render))
            }
            Action::ClinicalAllergyCancel => {
                self.current_view = ClinicalView::AllergyList;
                Ok(Some(Action::Render))
            }
            _ => Ok(None),
...
impl ClinicalComponent {
    ...
    fn handle_allergy_editor_input(&mut self, key: KeyEvent) -> Action {
        match key.code {
            ...
            KeyCode::Enter => {
                if !self.allergy_form.allergen.is_empty() {
                    return Action::ClinicalAllergySave;
                }
                Action::None
            }
            KeyCode::Tab => {
                self.allergy_form.active_field = self.allergy_form.active_field.next_field();
                Action::Render
            }
            KeyCode::BackTab => {
                self.allergy_form.active_field = self.allergy_form.active_field.prev_field();
                Action::Render
            }
            KeyCode::Up => {
                self.allergy_form.active_field = self.allergy_form.active_field.prev_field();
            ...
            KeyCode::Char(c) => {
                self.allergy_form.has_changes = true;
                match self.allergy_form.active_field {
                    AllergyFormField::Allergen => self.allergy_form.allergen.push(c),
                    AllergyFormField::AllergyType => self.allergy_form.allergy_type.push(c),
                    AllergyFormField::Severity => self.allergy_form.severity.push(c),
                    AllergyFormField::Reaction => self.allergy_form.reaction.push(c),
                    AllergyFormField::OnsetDate => self.allergy_form.onset_date.push(c),
                    AllergyFormField::Notes => self.allergy_form.notes.push(c),
                }
                Action::Render
            }
            KeyCode::Backspace => {
                ...
                match self.allergy_form.active_field {
                    AllergyFormField::Allergen => {
                        self.allergy_form.allergen.pop();
                    }
                    AllergyFormField::AllergyType => {
                        self.allergy_form.allergy_type.pop();
                    }
                    AllergyFormField::Severity => {
                        self.allergy_form.severity.pop();
                    }
                    AllergyFormField::Reaction => {
                        self.allergy_form.reaction.pop();
                    }
                    AllergyFormField::OnsetDate => {
                        self.allergy_form.onset_date.pop();
                    }
                    AllergyFormField::Notes => {
                        self.allergy_form.notes.pop();
                    }
                }
                Action::Render
            }
