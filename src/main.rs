...
use opengp_ui::ui::app::{
    AppError, AppEvent, AppState, DialogContent, GlobalState, RetryOperation, WorkspaceOpenIntent,
};
...
fn init(state: &mut AppState, _ctx: &mut GlobalState) -> Result<(), AppError> {
    if state.authenticated {
        ...
        spawn_load_practitioners(_ctx);
        spawn_refresh_appointments(_ctx, chrono::Utc::now().date_naive());
    }
    Ok(())
}
...
fn spawn_update_appointment_status(
    ...
    new_status: opengp_domain::domain::appointment::AppointmentStatus,
    user_id: uuid::Uuid,
    ...
    ctx.spawn_async(async move {
        let result = if let Some(svc) = service {
            let status_result = match new_status {
...
struct PendingRescheduleData {
    appointment_id: uuid::Uuid,
    new_date: Option<chrono::NaiveDate>,
    new_time: Option<chrono::NaiveTime>,
    practitioner_id: uuid::Uuid,
...
fn spawn_open_patient_from_appointment(
    ctx: &GlobalState,
    ...
    let service = ctx.patient_ui_service.clone();
    ctx.spawn_async(async move {
        let patient = if let Some(svc) = service {
            svc.get_patient(patient_id)
                .await
                .map(opengp_ui::ui::view_models::PatientListItem::from)
                .map_err(|e| e.to_string())
        } else {
            Err("No patient service".to_string())
        };
        Ok(Control::Event(AppEvent::PatientOpenedFromAppointment {
            patient,
            appointment_id,
            intent,
        }))
...
fn dialog_render(
    ...
    match dialog {
        ...
        DialogContent::AppointmentDetailModal(modal) => (&*modal).render(area, buf),
...
fn current_key_context(state: &AppState, ctx: &GlobalState) -> KeyContext {
    if !state.authenticated {
        return KeyContext::Global;
    }
    if rat_focus::HasFocus::is_focused(&state.tab_bar) {
        return KeyContext::TabBar;
    }
    ...
    if let Some(index) = ctx.dialogs.top::<DialogContent>() {
        if let Some(dialog) = ctx.dialogs.get::<DialogContent>(index) {
            match &*dialog {
                DialogContent::HelpOverlay(_) => return KeyContext::Global,
                DialogContent::PatientForm(form) if form.is_focused() => {
                    return KeyContext::PatientForm;
                }
                DialogContent::AppointmentForm(form) if form.is_focused() => {
                    return KeyContext::Schedule;
                }
    ...
    // Fall through to tab-based context
    match state.tab_bar.selected() {
        ...
        Tab::Schedule => match state.appointment_state.current_view {
            opengp_ui::ui::components::appointment::AppointmentView::Calendar => {
                KeyContext::Calendar
            }
            opengp_ui::ui::components::appointment::AppointmentView::Schedule => {
                KeyContext::Schedule
            }
        },
    }
...
fn toggle_help_dialog(state: &mut AppState, ctx: &GlobalState) {
    ...
    help_overlay.set_context(current_key_context(state, ctx));
    help_overlay.show();
...
fn render(
    ...
    if !state.authenticated {
        ...
        {
            use opengp_ui::ui::components::{PatientTab, PatientTabBar};
            let main_tab_w: u16 = 40;
            state.tab_bar.clone().render(
                Rect::new(layout[0].x, layout[0].y, main_tab_w.min(layout[0].width), 1),
                buf,
            );
            if !state.workspace_manager.workspaces.is_empty() {
                let patient_tabs: Vec<PatientTab> = state
                    .workspace_manager
                    .workspaces
                    .iter()
                    .map(|ws| PatientTab::new(ws.patient_id, ws.patient_snapshot.full_name.clone(), ws.colour))
                    .collect();
                let active_idx = state.workspace_manager.active_index.unwrap_or(0);
                let hovered = state.workspace_manager.hovered_tab.element_id;
                let patient_tab_area = Rect::new(
                    layout[0].x + main_tab_w,
                    layout[0].y,
                    layout[0].width.saturating_sub(main_tab_w),
                    1,
                );
                PatientTabBar::new(patient_tabs, active_idx, ctx.theme.clone())
                    .with_hovered(hovered)
                    .render(patient_tab_area, buf);
            }
        }

        match state.tab_bar.selected() {
            Tab::Schedule => {
                use opengp_ui::ui::components::appointment::AppointmentView;
                match state.appointment_state.current_view {
                    AppointmentView::Calendar => {
                        state.appointment_state.calendar.clone().render(layout[1], buf);
                    }
                    AppointmentView::Schedule => {
                        ...
                        let chunks = Layout::default()
                            ...
                            .split(layout[1]);
                        state.appointment_state.calendar.clone().render(chunks[0], buf);
                        let schedule = state.appointment_state.schedule.clone();
                        let mut appointment_state = &mut state.appointment_state;
                        ratatui::widgets::StatefulWidget::render(
                            schedule,
                            chunks[1],
                            buf,
                            &mut appointment_state,
                        );
                    }
                }
            }
            ...
            Tab::PatientWorkspace => {
                use opengp_ui::ui::components::clinical::ClinicalView;
                use opengp_ui::ui::components::{ClinicalMenuKind, ClinicalRow};
                use opengp_ui::ui::components::clinical::{
                    SocialHistoryComponent,
                    PatientSummaryComponent,
                };

                if let Some(workspace) = state.workspace_manager.active() {
                    let clinical_row_area = Rect::new(layout[1].x, layout[1].y, layout[1].width, 1);
                    let active_timer_text = workspace.clinical.as_ref().and_then(|c| {
                        c.consultations.active_timer_started_at.map(|started| {
                    ...
                    let active_menu_idx = workspace.active_clinical_menu.index();
                    ClinicalRow::new(ClinicalMenuKind::all(), active_menu_idx, workspace.colour, ctx.theme.clone())
                        .with_timer(active_timer_text)
                        .render(clinical_row_area, buf);
                    ...
                    if let Some(ref clinical) = workspace.clinical {
                        match &clinical.view {
                            ClinicalView::PatientSummary => {
                                PatientSummaryComponent::new(ctx.theme.clone())
                                    .render(content_area, buf);
                            }
                            ClinicalView::Consultations => {
                                clinical.consultations.consultation_list.clone().render(content_area, buf);
                            }
                            ClinicalView::Allergies => {
                                clinical.allergies.allergy_list.clone().render(content_area, buf);
                            }
                            ClinicalView::VitalSigns => {
                                clinical.vitals.vitals_list.clone().render(content_area, buf);
                            }
                            ClinicalView::MedicalHistory => {
                                clinical.medical_history.medical_history_list.clone().render(content_area, buf);
                            }
                            ClinicalView::FamilyHistory => {
                                clinical.family_history.family_history_list.clone().render(content_area, buf);
                            }
                            ClinicalView::SocialHistory => {
                                SocialHistoryComponent::new(
                                    ctx.theme.clone(),
                                    &ctx.social_history_config,
                                ).render(content_area, buf);
                            }
                        }
                    }
...
fn build_focus(state: &AppState) -> Focus {
    ...
    builder
        ...
        // login_screen is excluded — it manages its own Tab key navigation internally
        ...
        if let Some(action) = modal.handle_key(key) {
            match action {
                ...
                AppointmentDetailModalAction::ViewClinicalNotes => {
                    ...
                    let appointment_id = modal.appointment_id();
                    spawn_open_patient_from_appointment(
                        ctx,
                        patient_id,
                        appointment_id,
                        WorkspaceOpenIntent::ViewClinical,
                    );
                    return DialogKeyResult::HandledClose;
                }
                AppointmentDetailModalAction::StartConsultation => {
                    ...
                    spawn_open_patient_from_appointment(
                        ...
                        patient_id,
                        appointment_id,
                        WorkspaceOpenIntent::StartConsultation,
                    );
                    return DialogKeyResult::HandledClose;
                }
    ...
    match event {
        AppEvent::Term(term_event) => match term_event {
            Event::Key(key) => {
                ...
                let has_dialog = ctx.dialogs.top::<DialogContent>().is_some();

                if !has_dialog && matches!(focus.handle(term_event, Regular), Outcome::Changed) {
                            ...
                            ScheduleAction::SelectAppointment(id) => {
                                ...
                                if let Some(apt) = state
                                    ...
                                    .and_then(|d| {
                                        d.practitioners
                                            .iter()
                                            .flat_map(|ps| ps.appointments.iter())
                                            .find(|a| a.id == id)
                                    })
                                    .cloned()
                                {
                                    let modal = AppointmentDetailModal::new(apt, ctx.theme.clone());
                                    push_dialog(ctx, DialogContent::AppointmentDetailModal(modal));
                                }
                            }
                ...
                match key.code {
                    ...
                    _ => {
                        let action = KeybindRegistry::global()
                            .lookup(*key, current_key_context(state, ctx))
                            ...
                            match action {
                                ...
                                Action::Escape => {
                                        ...
                                        .is_none()
                                    {
                                        state.appointment_state.current_view =
                                            AppointmentView::Calendar;
                                        build_focus(state).focus(&state.appointment_state.calendar);
                                        changed = true;
                                    }
                                    if changed {
                                        refresh_status_and_context(state, ctx);
            ...
            Event::Mouse(mouse) => {
                handle_mouse_event(state, ctx, *mouse);
                Ok(Control::Changed)
            }
            Event::Resize(w, h) => {
            ...
            match result {
                Ok(schedule) => {
                    let practitioner_colours = [
                        ctx.theme.colors.appointment_confirmed,
                        ctx.theme.colors.appointment_in_progress,
                        ctx.theme.colors.appointment_arrived,
                        ctx.theme.colors.appointment_scheduled,
                        ctx.theme.colors.primary,
                        ctx.theme.colors.secondary,
                    ];
                    state
                        .appointment_state
                        .load_schedule_data_with_colours(schedule.clone(), &practitioner_colours);
                    state.status_bar.clear_error();
                }
                Err(err) => state.status_bar.set_error(Some(err.clone())),
            }
        ...
        AppEvent::PatientOpenedFromAppointment {
            patient,
            appointment_id,
            intent,
        } => {
            match patient {
                Ok(patient_list_item) => {
                    use opengp_ui::ui::components::clinical::ClinicalState;
                    use opengp_ui::ui::components::clinical::ClinicalView;
                    let patient_id = patient_list_item.id;
                    match state.workspace_manager.open_patient(patient_list_item.clone()) {
                        Ok(_) => {
                            state.tab_bar.select(Tab::PatientWorkspace);
                            if let Some(workspace) = state.workspace_manager.active_mut() {
                                workspace.active_subtab = opengp_ui::ui::components::SubtabKind::Clinical;
                                if workspace.clinical.is_none() {
                                    let clinical = ClinicalState::new(
                                        ctx.theme.clone(),
                                        ctx.healthcare_config.clone(),
                                        ctx.allergy_config.clone(),
                                        ctx.clinical_config.clone(),
                                        ctx.social_history_config.clone(),
                                    );
                                    workspace.clinical = Some(clinical);
                                }
                                if let Some(ref mut clinical) = workspace.clinical {
                                    clinical.selected_patient_id = Some(patient_id);
                                    clinical.active_appointment_id = Some(*appointment_id);
                                    clinical.view = ClinicalView::Consultations;
                                    if *intent == WorkspaceOpenIntent::StartConsultation {
                                        clinical.set_active_timer_started_at(chrono::Utc::now());
                                        clinical.consultations.active_appointment_id = Some(*appointment_id);
                                    }
                                }
                            }
                            state.status_bar.clear_error();
                        }
                        Err(e) => state.status_bar.set_error(Some(e.to_string())),
                    }
                }
                Err(err) => state.status_bar.set_error(Some(err.clone())),
            }
...
fn handle_mouse_event(state: &mut AppState, ctx: &mut GlobalState, mouse: crossterm::event::MouseEvent) {
    ...
    if state.tab_bar.handle_mouse(mouse, main_tab_bar_area).is_some() {
        refresh_status_and_context(state, ctx);
        return;
    }
    ...
    if !state.workspace_manager.workspaces.is_empty() {
        ...
        if let Some(workspace) = state.workspace_manager.active_mut() {
            ...
            state.hovered_clinical_menu = clinical_row.hovered_index();
        }
    }

    let clinical_row_offset: u16 = if state.workspace_manager.active().is_some() { 2 } else { 1 };
    ...
    if state.tab_bar.selected() == Tab::PatientSearch {
        ...
        if let Some(action) = state.patient_list.handle_mouse(mouse, content_area) {
            match action {
                PatientListAction::OpenPatient(id) => {
                    spawn_load_patient_for_edit(ctx, id);
                }
                PatientListAction::SelectionChanged
                | PatientListAction::FocusSearch
    ...
    if state.tab_bar.selected() == Tab::Schedule {
        ...
        match state.appointment_state.current_view {
            AppointmentView::Calendar => {
                    ...
                    .handle_mouse(mouse, appointment_content_area)
                {
                    match action {
                        CalendarAction::SelectDate(date) => {
                            state.appointment_state.selected_date = Some(date);
                            state.appointment_state.current_view = AppointmentView::Schedule;
                            spawn_refresh_appointments(ctx, date);
                            refresh_status_and_context(state, ctx);
                        }
                        CalendarAction::FocusDate(_)
            ...
            AppointmentView::Schedule => {
                ...
                if let Some(action) = state.appointment_state.handle_mouse(mouse, chunks[1]) {
                    build_focus(state).focus(&state.appointment_state);
                    match action {
                        ScheduleAction::SelectPractitioner(id) => {
                            state.appointment_state.selected_practitioner = Some(id);
                        }
    ...
    if state.tab_bar.selected() == Tab::PatientWorkspace {
        if let Some(workspace) = state.workspace_manager.active_mut() {
            if let Some(ref mut billing_state) = workspace.billing {
                ...
                if matches!(mouse.kind, MouseEventKind::Moved) {
                    ...
                    if let Some(last) = state.last_billing_render {
                        if last.elapsed() < Duration::from_millis(16) {
                            return;
                        }
                    }
                    state.last_billing_render = Some(std::time::Instant::now());
                }

                match billing_state.view {
                    BillingView::ClaimList => {
                        let mut claim_list =
                            ClaimList::new(billing_state.claims.clone(), ctx.theme.clone());
                        ...
                        if let Some(action) = claim_list.handle_mouse(mouse, billing_content_area) {
                            match action {
                                opengp_ui::ui::components::billing::ClaimListAction::Select(_) => {
                    ...
                    BillingView::InvoiceList => {
                        let mut invoice_list = InvoiceList::new(ctx.theme.clone());
                        ...
                        {
                            match action {
                                opengp_ui::ui::components::billing::InvoiceListAction::Select(_) => {
                                    billing_state.invoice_selected_index =
                                        invoice_list.selected_index;
                                }
                                _ => {}
                            }
                        }
                    ...
                    BillingView::PaymentList => {
                        ...
                        {
                            match action {
                                opengp_ui::ui::components::billing::PaymentListAction::Select(_) => {
                                    billing_state.payment_selected_index =
                                        payment_list.selected_index;
                                }
                                _ => {}
                            }
                        }
            ...
            if let Some(ref mut clinical_state) = workspace.clinical {
                ...
                match clinical_state.view {
                    opengp_ui::ui::components::clinical::ClinicalView::Consultations => {
                        let _ = clinical_state
                            .consultations
                            .consultation_list
                            .handle_mouse(mouse, clinical_content_area);
                    }
                    opengp_ui::ui::components::clinical::ClinicalView::VitalSigns => {
                        let _ = clinical_state
                            .vitals
                            .vitals_list
                            .handle_mouse(mouse, clinical_content_area);
                    }
                    opengp_ui::ui::components::clinical::ClinicalView::Allergies => {
                        let _ = clinical_state
                            .allergies
                            .allergy_list
                            .handle_mouse(mouse, clinical_content_area);
                    }
                    opengp_ui::ui::components::clinical::ClinicalView::MedicalHistory => {
                        let _ = clinical_state
                            .medical_history
                            .medical_history_list
                            .handle_mouse(mouse, clinical_content_area);
                    }
                    opengp_ui::ui::components::clinical::ClinicalView::FamilyHistory => {
                        let _ = clinical_state
                            .family_history
                            .family_history_list
                            .handle_mouse(mouse, clinical_content_area);
